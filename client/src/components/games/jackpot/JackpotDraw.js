...
const JackpotDraw = ({ jackpot }) => {
  ...
  if (offset === 0) {
    if (jackpot.winnerOffset !== 0) {
      setOffset(jackpot.winnerOffset * 201 - getCross())
    }
  }
  ...
  const drawItems = jackpot.gameTable.map((item, index) => (
    <div
      className="jackpot__draw-item"
      key={index}
      style={{ backgroundColor: item.color }}
    >
      <div
        className="jackpot__draw-image"
        style={{ backgroundImage: `url(avatars/${item.avatar})` }}
      ></div>
      <div className="jackpot__draw-body">
        <div className="jackpot__draw-nick">{item.nick}</div>
        <div className="jackpot__draw-cash">
          {Math.round(item.chance * 10000000) / 100000}%
        </div>
      </div>
    </div>
  ))
  ...
  return (
    <div className="jackpot__draw">
      ...
      <div
        style={
          offset
            ? { left: `-${offset}px`, transition: 'left 15s ease' }
            : { left: `${offset}px`, transition: 'left 0s ease' }
        }
        className="jackpot__draw-container"
      >
        {drawItems}
      </div>
    </div>
