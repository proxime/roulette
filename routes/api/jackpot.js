...
router.get('/', async (req, res) => {
  try {
    const lastGames = await Jackpot.find({ current: false })
      .sort({ _id: -1 })
      .limit(5)
      .populate('winner')
...
router.post(
  '/',
  [auth, [check('luckycoins', 'Wprowadź prawidłową liczbę').isInt()]],
  async (req, res) => {
    ...
    try {
      if (luckycoins <= 0) {
        return res
          .status(400)
          .json({ errors: [{ msg: 'Wprowadź prawidłową liczbę' }] })
      }
      ...
      if (user.luckycoins < luckycoins) {
        return res
          .status(401)
          .json({ errors: [{ msg: 'Nie posiadasz tyle Luckycoins!' }] })
      }
      user.luckycoins -= Number(luckycoins)
      await user.save()

      if (jackpot) {
        const isInGame = jackpot.players.filter(
          player => player.userId == user.id,
        )
        jackpot.cash += Number(luckycoins)
        if (!isInGame.length)
          jackpot.players.push({ userId: user.id, cash: Number(luckycoins) })
        else {
          jackpot.players.map(player =>
            player.userId == user.id
              ? (player.cash += Number(luckycoins))
              : player,
          )
        }
        await jackpot.save()
